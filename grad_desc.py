...
def polynomial_learning_rate_scheduling(
    start_learning_rate: float, alpha: float, beta: float
):
...
def gradient_descend(
    ...
    *,
    ...
    for _ in range(max_iter):
        grad = jac(path[-1])
        new_point = (
            path[-1] - learning_rate_function(0, 0.1, func, path[-1], grad) * grad
        )
...
def gradient_and_newton(
    ...
    newton_res = newton_minimize_function(
        func,
...
def stochastic_gradient_descent(
    ...
    *,
    ...
    if (stop_grad_norm is not None) and stop_grad_norm < 0:
        raise ValueError(
            "Условие останова по норме градиента должно быть положительным"
        )
    if (stop_epoch_number is not None) and stop_epoch_number <= 0:
        raise ValueError(
            "Условие останова по количеству эпох должно быть положительным"
        )
    if batch_size <= 0:
    ...
    loss_calculator = (
        lambda vect: sum((objective_func_term(vect, i) for i in range(data_size)))
        / data_size
    )
    path = [start]
