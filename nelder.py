...
def add_sim():
    ...
    ns = {}
    exec(
        nelder_code, scipy.optimize._optimize.__dict__, ns
    )  # exec(code, globals, locals)

    exec(
        minimize_code, scipy.optimize._minimize.__dict__ | ns, ns
    )  # ns = {'_minimize_neldermead': ...}

    scipy.optimize.minimize = ns[
        "minimize"
    ]  # ns = {'_minimize_neldermead': ..., 'minimize': ...}


def scipy_nelder_mead(
    f: RFunction, x0: Point, max_iterations=1000, verbose=True, *args, **kwargs
):
    add_sim()
    path = []
    ...
    res = scipy.optimize.minimize(
        ...
        *args,
        method="Nelder-Mead",
        tol=1e-6,
        options=dict(disp=verbose, maxiter=max_iterations),
        callback=callback,
        **kwargs
    )
